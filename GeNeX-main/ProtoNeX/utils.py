...
def evaluate_ensemble(prototypes, weights, test_loader):
    """Evaluate weighted ensemble on test set"""
    prototypes = [p.to(device) for p in prototypes]
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(test_loader, desc="Evaluating Ensemble"):
            ...
            ensemble_probs = 0
            for model, weight in zip(prototypes, weights):
                outputs = model(inputs)
                probs = FF.softmax(outputs, dim=1)
                ensemble_probs += weight * probs
 
            all_preds.append(ensemble_probs.argmax(dim=1).cpu())
    ...
    gm = Gm_Score(preds, labels)
 
    print(f"\nTest Results - GM: {gm:.4f}")
    return gm
